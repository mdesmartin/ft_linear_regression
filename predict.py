...
def load_coefficients(filename: str = 'coefficients.csv') -> tuple[float, float, float, float]:
    try:
...
def normalize_input(mileage: float, x_min: float, x_max: float) -> float:
    if x_min == x_max:
        raise ValueError("x_min and x_max cannot be the same for normalization.")
